...
public class GuiItemDurability {
    ...
    private void renderItemBreakingOverlay(MatrixStack matrices, ItemStack itemStack, long timeDelta) {
        ...
        DrawableHelper.fill(matrices, 0, 0, mainWindow.getScaledWidth(), mainWindow.getScaledHeight(), 0xff0000 + ((int) (alpha * 128) << 24));
        ...
        for (ItemIndicator item : items) {
            if (item != null && !item.isEmpty() && item.isItemStackDamageable()) {
                ...
                if (reallyDraw) {
                    ...
                    fontRenderer.draw(stack, displayString, numberPos != RenderPos.right ? xpos : xpos + iconWidth + spacing, ypos + result.height + fontRenderer.fontHeight / 2f + (numberPos == RenderPos.over ? 10 : 0), color);
                }
                result.height += 16;
            }
        }
